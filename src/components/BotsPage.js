...
function BotsPage() {
  ...
  function handleBotActionClick(bot, action) {
    switch (action) {
      ...
      default:
        // Handle unexpected action here, or simply ignore it
        break;
    }
  }
  ...
  function botsList(botsArray) {
    return botsArray.map((bot) => (
      <BotCard key={bot.id} bot={bot} handleBotActionClick={handleBotActionClick} />
    ));
  }
  ...
  return (
    <div>
      ...
      {showBotSpecs ? (
        <BotSpecs bot={showBotSpecs} handleBotActionClick={handleBotActionClick} alreadyListedBot={alreadyListedBot(showBotSpecs)} />
      ) : (
